...
"""Evidence CLI for host mode: the harness drives the evidence layer from the shell.
  ...
  python evidence_cli.py ingest --task galaxy_xsurvey --call 1
...
then `ingest` consumes the receipt written by the CLI's `view_image` tool. It does not trust a hand-written
description. That receipt-backed text is what the agent reasons over.
"""
...
def main():
    ...
    p.add_argument("--answers", default="{}", help="deprecated: receipt observations are ingested directly")
