...
def new(width="col", ratio=None, nrows=1, ncols=1, **kw):
    ...
    ratio = height/width. The default is the 10:4.3 house shape (2026-09-02):
    a single-column figure prints 0.43 as tall as it is wide, and a 'wide'
    figure doubles the WIDTH at the SAME printed height, so going wide never
    eats the page. A sparse chart (a handful of bars or points) should pass a
    SMALLER ratio, never a larger one. The value is CLAMPED between "every
    panel prints at least an inch tall" and "no page-hogging tower".
    Returns (fig, ax)."""
    w = WIDE_W if width == "wide" else COL_W
    scale = COL_W / w                       # 'wide' keeps a column figure's printed height
    if ratio is None:
        ratio = 0.43 * scale
    lo = max(1.00 * nrows / w, 0.14)                      # per-panel printed height >= 1 inch
    hi = max(lo, min(1.15, 0.9 * nrows / max(1, ncols)) * scale)
    r = min(max(float(ratio), lo), hi)
    if abs(r - float(ratio)) > 1e-6:
        print("[figtools] ratio %.2f clamped to %.2f (%dx%d panels must print dense: no ribbons, no towers)"
