...
"""
import os, re, sys, json, types, hashlib
...
def validate_receipt(td, call, item):
    """Validate the receipt structure and bind it to the current pixels, question, and observation."""
    receipt = (call.get("receipts") or {}).get(str(item["id"]))
    if not isinstance(receipt, dict):
        return None, "vision request %s has no view_image receipt" % item["id"]
    image = item["image"] if os.path.isabs(item["image"]) else os.path.join(td, item["image"])
    if not os.path.isfile(image):
        return None, "vision request %s image is missing: %s" % (item["id"], image)
    observation = str(receipt.get("observation") or "").strip()
    checks = {
        "receipt_id": bool(receipt.get("receipt_id")),
        "provider": bool(receipt.get("provider")),
        "model": bool(receipt.get("model")),
        "image_sha256": receipt.get("image_sha256") == _sha256_bytes(open(image, "rb").read()),
        "question_sha256": receipt.get("question_sha256") == _sha256_bytes(str(item["question"]).encode()),
        "observation_sha256": receipt.get("observation_sha256") == _sha256_bytes(observation.encode()),
        "observation": bool(observation),
    }
    bad = [name for name, ok in checks.items() if not ok]
    if bad:
        return None, "vision request %s has an invalid receipt: %s" % (item["id"], ", ".join(bad))
    return receipt, None
...
def ingest(td, call_id, answers):
    ...
    text, missing, grounded_answers = call["draft"], [], {}
    for item in call["pending"]:
        receipt, error = validate_receipt(td, call, item)
        if error:
            missing.append(error); continue
        observation = str(receipt["observation"]).strip()
        supplied = answers.get(str(item["id"])) or answers.get(item["id"])
        if supplied and str(supplied).strip() != observation:
            raise SystemExit("answer for vision request %s differs from its view_image receipt; omit --answers "
                             "and ingest the recorded observation" % item["id"])
        grounded_answers[str(item["id"])] = observation
        text = text.replace(PLACEHOLDER % item["id"], observation)
    if missing:
        raise SystemExit("cannot ingest perception: %s. Call OmniScientist view_image with the pending image and exact "
                         "question first." % "; ".join(missing))
    call["result"], call["status"], call["answers"] = text, "done", grounded_answers
