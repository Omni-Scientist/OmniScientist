    ...
    for p in sorted(glob.glob(os.path.join(hb.host_dir(td), "calls", "call_*.json"))):
        ...
        if c.get("pending"):
            if c.get("status") == "done":
                # Host mode: the host read the pixels itself and ingested its own observations via
                # `evidence_cli.py ingest --answers`; there is no view_image receipt to validate. The
                # CLI-side skill binds receipts here (hostbridge.validate_receipt); that is the one
                # deliberate fork between the two skills.
                ingested += 1
            else:
