...
export function discoverArtifacts(
  ...
  for (const [index, figure] of (manifest.inputs?.figures ?? []).entries()) {
    ...
    artifacts.push({
      ...
      title: basename(registered.relative),
      path: registered.relative,
      detail: `论文主图 ${index + 1} · ${fileSize(statSync(registered.absolute).size)}`,
      updatedAt: timeLabel(registered.absolute),
      order: 20 + index,
  ...
  if (tex) {
    ...
    artifacts.push({
      ...
      path: tex.relative,
      detail: `论文 LaTeX · ${source.split(/\r?\n/).length} 行`,
  ...
  if (zip) {
    ...
    artifacts.push({
      ...
      path: zip.relative,
      detail: `Overleaf 上传包 · ${kb} KB · 含 .tex、参考文献和全部图`,
  ...
  if (pdf) {
    ...
    artifacts.push({
      ...
      path: pdf.relative,
      detail: `${previews.length || "?"} 页论文 · ${fileSize(statSync(pdf.absolute).size)}`,
