...
staged.py. It depends ONLY on pipeline.py for the low-level LLM transport (the OpenAI-compatible client + image
...
PERCEIVER = os.environ.get("OMNIST_PERCEIVER") or os.environ.get("OMNIST_MODEL", "claude-sonnet-5")   # agent brain + eye; follows OMNIST_MODEL, or OMNIST_PERCEIVER for a dedicated vision model
...
def _idea_exec(task):
    ...
    def _look(args, state, log):
        ...
        for fid in files:
            ...
            ans = pipeline.chat([{"type": "text", "text": q}, img_block(path)], PERCEIVER, 400)
            state["img_used"] += 1
...
def stage1_ideation(task):
    ...
    obj = agent_loop(system, task_msg, _idea_tools(ptools), exec_fn, PERCEIVER, _idea_done,
    ...
    md = ["# Stage 1 -- Ideation (AGENTIC: tool-using, real-search-grounded)", "",
          "_task: %s | %s %ss | property: %s | brain: %s_" % (task, n, subject, prop, PERCEIVER), ""]
    if obj.get("_failed"):
...
def stage2_experiment(task):
    ...
    obj = agent_loop(system, task_msg, _exp_tools(ptools), exec_fn, PERCEIVER, _exp_done,
    ...
    md = ["# Stage 2 -- Experiment (AGENTIC: methods + real code execution)", "",
          "_task: %s | role: %s | brain: %s_" % (task, role, PERCEIVER), ""]
    if obj.get("_failed"):
...
def stage3_writeup(task):
    ...
    catalog, bib = _gather_citations_retry(pipeline.chat, PERCEIVER, C, idea_text, obs)
    ...
    thesis = _plan_thesis(pipeline.chat, PERCEIVER, C, exp, idea)
    if thesis:
    ...
    out = writer.write_paper(pipeline.chat, PERCEIVER, C, exp, idea, catalog,
                             _scrub_rs, _trim_incomplete, exp.get("demoted", []), style, figmap, thesis=thesis)
    title = _gen_title(pipeline.chat, PERCEIVER, C, exp, thesis=thesis)
    ...
    tex = _structure_sections(pipeline.chat, PERCEIVER, tex)
    ...
    tex = _dedash_tex(_polish_tex(pipeline.chat, PERCEIVER, tex, _known, _gf, _forbidden, _real_models))
    tex = _annotate_exclusions(tex, exp, pipeline.chat, PERCEIVER)   # #1a: state which categories a test used
    ...
    md = ["# Stage 3 -- Writeup (research paper -> PDF)", "",
          "_task: %s | style: %s | figures: %d | references: %d | brain: %s_" % (task, style, len(figblocks), len(catalog), PERCEIVER), "",
...
def main():
    ...
    if _co:
        try:
            json.dump({"task": a.task, "stage": a.stage, "model": PERCEIVER,
